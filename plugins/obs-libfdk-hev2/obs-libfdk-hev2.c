...
static obs_properties_t *libfdk_properties(void *unused)
{
	...
	obs_properties_add_int(props, "bitrate",
			obs_module_text("Bitrate"), 18, 40, 2);
...
static void *libfdk_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	blog(LOG_INFO, "libfdk_aac_v2 encoder created");

	blog(LOG_INFO, "libfdk_aac_v2 bitrate: %d, channels: %d",
...
fail:
	...
	blog(LOG_WARNING, "libfdk_aac_v2 encoder creation failed");
...
static void libfdk_destroy(void *data)
{
	...
	blog(LOG_INFO, "libfdk_aac_v2 encoder destroyed");
}
...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("obs-libfdk-hev2", "en-US")
