...
static obs_properties_t *libfdk_properties(void *unused)
{
	...
	obs_properties_add_int(props, "bitrate",
			obs_module_text("Bitrate"), 42, 64, 2);
...
{
	obs_data_set_default_int(settings, "bitrate", 50);
...
static void *libfdk_create(obs_data_t *settings, obs_encoder_t *encoder)
{
	...
	blog(LOG_INFO, "libfdk_aac_v1 encoder created");

	blog(LOG_INFO, "libfdk_aac_v1 bitrate: %d, channels: %d",
...
fail:
	...
	blog(LOG_WARNING, "libfdk_aac_v1 encoder creation failed");
...
static void libfdk_destroy(void *data)
{
	...
	blog(LOG_INFO, "libfdk_aac_v1 encoder destroyed");
}
...
OBS_DECLARE_MODULE()
OBS_MODULE_USE_DEFAULT_LOCALE("obs-libfdk-hev1", "en-US")
