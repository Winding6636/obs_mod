...
static void CreateOBS()
{
	if (!obs_startup("en", nullptr))
		throw "Couldn't create Background Color";
	...
	if (obs_reset_video(&ovi) != 0)
		throw "Couldn't initialize Background Color";
}
...
- (void)applicationDidFinishLaunching:(NSNotification*)notification
{
	...
	try {
		...
		win = [[NSWindow alloc]
			...
			defer:NO];
		if (!win)
			throw "Could not create Background Color window";

		view = [[NSView alloc] initWithFrame:content_rect];
		if (!view)
			throw "Could not create Background Color view";
