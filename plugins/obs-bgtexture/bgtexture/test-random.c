...
static const char *bgcolor_getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return "20x20 Random Pixel Texture Source (Test)";
}

static void bgcolor_destroy(void *data)
{
	struct bgcolor_tex *rt = data;
...
static void *bgcolor_create(obs_data_t *settings, obs_source_t *source)
{
	struct bgcolor_tex *rt = bzalloc(sizeof(struct bgcolor_tex));
	rt->source = source;
...
struct obs_source_info test_bgcolor = {
	.id           = "bgcolor",
	.type         = OBS_SOURCE_TYPE_INPUT,
	.output_flags = OBS_SOURCE_ASYNC_VIDEO,
	.get_name     = bgcolor_getname,
	.create       = bgcolor_create,
	.destroy      = bgcolor_destroy,
};
